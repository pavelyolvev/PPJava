...
public class Main {
    ...
    static void openFilmCollection() throws FilmCollection.FilmColException {
        ...
        interAr.add(StreamMethods.createInstance(title, credits, duration, true));
    ...
    static void openTheaterCollection() throws TheaterCollection.ThColException {
        ...
        interAr.add(StreamMethods.createInstance(title, credits, duration, false));
    }
    ...
    private static void lab6(){
        Random rand = new Random();
        //ITERATOR
        CollectionMethods iterCol;
        if(interAr.size() != 0)
            iterCol = interAr.get(rand.nextInt(interAr.size()));
        else throw new RuntimeException("No collections");
        System.out.println("Duration from collection: " + iterCol + "\tusing iterator:");
        for (Integer integer : iterCol) System.out.print(integer + "\t");


        //UnModifable collection
        CollectionMethods unModCol;
        if(interAr.size() != 0)
            unModCol = interAr.get(rand.nextInt(interAr.size()));
        else throw new RuntimeException("No collections");
        unModCol = StreamMethods.unmodifableColMethods(unModCol);
        unModCol.setElement(0, 0);
    }
}
